...
const RegisterOrderForm = ({ products = [], providers = [], users = [] }) => {
	...
	const disableForm = () => {
		if (
			...
			!form.provider_id ||
			!form.user_id ||
			form.products.find((el) => el.id === "" || JSON.parse(el.quantity) === 0)
		)
			return true;
		else return false;
	};
	...
	return (
		...
		>
			<form onSubmit={handleSubmit}>
				...
				<SelectField
					value={form.user_id}
					options={users.map(({ id, first_name, last_name }) => ({
						value: id,
				...
				<Flex flexDir="column" justifyContent="center">
					...
					{form.provider_id && (
						<Box w="full">
							{form.products.map(({ quantity, id }, index) => (
								<Card
									key={`box-${index}`}
									bg="gray.100"
									p="8"
